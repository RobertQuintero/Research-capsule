...
export class AssignedResearchComponent implements OnInit {
  ...
  openRevise(id: number) {
    this.capsuleToRejectId = id
    this.reviseModalShow = true
  }

  closeReviseModal() {
    this.reviseModalShow = false
    this.capsuleToRejectId = undefined
  }

  rejectCapsule() {
    this.isLoading = true
    const dataObject = {
      capsule_id: this.capsuleToRejectId,
    ...
    this.facultyService.rejectCapsule(dataObject).subscribe((response) => {
      ...
      this.dataSource.sort = this.sort;
      this.isLoading = false
      this._snack.success(`Capsule with the id of ${this.capsuleToRejectId} has been rejected successfully`)
    }, (err) => {
      console.log(err)
      this._snack.error(err.error.message)
      this.isLoading = false
